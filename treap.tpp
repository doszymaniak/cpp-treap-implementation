...
template <typename T> int Treap<T>::Node::random_priority() {
...
template <typename T>
Treap<T>::Node::Node(const T &k)
    : key(k), priority(random_priority()), left(nullptr), right(nullptr) {}

template <typename T>
std::pair<typename Treap<T>::Node *, typename Treap<T>::Node *>
Treap<T>::Node::split(Node *t, const T &el) {
  if (t == nullptr)
...
template <typename T> Treap<T>::Treap() : root(nullptr){};
...
template <typename T> Treap<T>::Treap(Treap<T> &&other) : root(other.root) {
  other.root = nullptr;
}

template <typename T> void Treap<T>::Node::destroy(Treap<T>::Node *n) {
  if (n == nullptr)
    return;
  destroy(n->left);
  destroy(n->right);
...
template <typename T> Treap<T>::~Treap() {
  Node::destroy(root);
  root = nullptr;
}

template <typename T> Treap<T> &Treap<T>::operator=(const Treap &other) {
  if (this == &other)
    return *this;
...
template <typename T> Treap<T> &Treap<T>::operator=(Treap &&other) {
  if (this == &other)
    return *this;
  Node::destroy(root);
  root = other.root;
...
template <typename T> void Treap<T>::insert(const T &el) {
  root = Node::insert(el, root);
}

template <typename T> void Treap<T>::remove(const T &el) {
  root = Node::remove(el, root);
}

template <typename T> bool Treap<T>::find(const T &el) const {
  return Node::find(el, root) != nullptr;
}

template <typename T> const T &Treap<T>::find_minimum() const {
...
template <typename T> const T &Treap<T>::find_maximum() const {
...
template <typename T> void Treap<T>::remove_minimum() {
  root = Node::remove_minimum(root);
}

template <typename T> void Treap<T>::remove_maximum() {
  root = Node::remove_maximum(root);
}

template <typename T> void Treap<T>::Node::inorder(std::vector<T> &vec) const {
  if (left)
    left->inorder(vec);
  vec.push_back(key);
  if (right)
...
template <typename T> std::vector<T> Treap<T>::to_vector() const {
  std::vector<T> vec;
  if (root)
    root->inorder(vec);
  return vec;
